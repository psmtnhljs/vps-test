...
创建转发时可以选择 TCP 或 UDP、本地监听端口、目标端口以及本机出站 IP。脚本会区分外网地址和内网地址，并根据目标地址族自动匹配 `proxy_bind`：

- 菜单 2、3 默认创建外网静态 IP 或外网域名转发，只显示外网出站地址。
- 菜单 4 单独创建内网 IP 转发，只接受 RFC1918/CGNAT IPv4 或 ULA IPv6 目标地址，并只显示本机内网地址。
- 纯 IPv4 服务器不显示 IPv6 地址；纯 IPv6 服务器不允许创建 IPv4 服务转发。
- 双栈服务器可分别选择 IPv4 或 IPv6 出站地址。
- 多 IP 服务器会按外网/内网分类列出地址，创建时明确选择实际使用的地址。
- NAT VPS 如果网卡只有内网地址，脚本会通过公网探测识别出口 IP；公网出口 IP 只用于显示，外网转发会省略 `proxy_bind`，交给系统路由和上游 NAT 自动选择源地址。
